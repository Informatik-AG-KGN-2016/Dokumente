...
class Chat():
    ...
    def launch(self):
        ...
        self.__my_socket.send(create_packet_2(self.__nickname, "login"))
    ...
    def user_input_runnable(self):
        while self.__running:
            for line in sys.stdin:
                ...
                if line == "\\end":
                    json_string = create_packet_2(self.__nickname, "logout")
                    self.__my_socket.send(json_string)
                    self.__my_socket.close()
                    print("\nAusgeloggt!\n")
                    self.__running = False
                else:
                    json_string = create_packet_1(self.__nickname, line)
